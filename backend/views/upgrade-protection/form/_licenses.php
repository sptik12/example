...
echo GridView::widget([
	...
	'columns' => [
		[
			...
			'detailUrl' => Url::to(['/license/index-view', 'buttons' => 0]),
		...
		[
			...
			'value' => function ($model) {
				return $model->getLicenseTypeName();
			},
		],
		[
			'label' => Yii::t('app', 'AMS Date'),
			'content' => function ($model) use ($form) {
				return $this->render('_license', ['model' => $model, 'form' => $form]);
			},
			'contentOptions' => ['class' => 'text-nowrap ams-date'],
		],
		[
